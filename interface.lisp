...
(defun attendance-student (studentid)
  ...
  (let  (attendance ; Attendance Note
    ...
    (setf new-attendance
          (prompt-read (concatenate 'string "Attendance (" attendance ")")))
    ...
    (delete-rows :from *attendance*
		 :where (matching *attendance*
				  ...
				  :date *date*))
    ;;Insert new record
    (insert-row (list :studentid studentid :courseid *courseid*
...
(defun attendance-student-nonote (studentid)
  "Interactively sets the attendance for a particular student, based on global \
class, without modifying the note"
  (let (attendance ; Attendance Note
	note ; General note
	new-attendance; Input from user
