;;; -*- Mode: Lisp -*-
;;;
;;; $Header: /home/gene/library/website/docsrc/pdl/RCS/time.lisp,v 1.4 2005/02/14 15:22:09 gene Exp $
;;;
...
;AJA: Replaced the above with the below for simplicity's sake
...
(defvar *zones*
  (let ((ht (make-hash-table :test #'equal)))
    (setf
     ...
     (gethash "GMT" ht) 0
     (gethash "MDT" ht) -6   ;AJA: Mountain Daylight (CAN)
     (gethash "MST" ht) -7   ; AJA: Mountain Standard (CAN)
...
(defvar *default-recognizers*
      (list (make-fmt-recognizer "%d %b %Y")
	    (make-fmt-recognizer "%d &b")
            (make-fmt-recognizer "%Y-%m-%dT%H:%M:%S")
